...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = Observable.create[String]((observer: Observer[String]) => {
      obs subscribe ( term => observer.onNext(term.replaceAll(" ", "_")),
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs.map(s => Success(s)).onErrorReturn(t => Failure(t))
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = Observable.create[T]((observer: Observer[T]) => {

      val timeout = Observable.timer(totalSec seconds)
      timeout subscribe (_=>observer.onCompleted(), t=> {}, ()=>{})

      obs subscribe ( term => observer.onNext(term),
        t => observer.onError(t),
        () => observer.onCompleted()
        )
    });
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] =
      ...
      Observable.create[Try[S]]( (observer: Observer[Try[S]]) => {

        def onSuccess(v: T): Unit = {
          requestMethod(v) subscribe(
            v => observer.onNext(Success(v)),
